...
import { Heart, Facebook, Twitter, Instagram, Linkedin, Loader2, ArrowRight } from "lucide-react";

const API = `${process.env.REACT_APP_BACKEND_URL}/api`;

const NAV_LINKS = ["about", "mission", "programs", "impact", "gallery", "volunteer", "donate", "contact"];
const PROGRAM_LINKS = ["Digital Literacy", "Free Education", "Health Camps", "Food Distribution", "Tree Plantation", "Skill Development"];
...
export default function Footer() {
  ...
  const subscribe = async (e) => {
    ...
    try {
      ...
      toast.success("Subscribed! Look out for our monthly stories.");
      setEmail("");
    } catch (err) {
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-10">
          <div className="lg:col-span-4">
            ...
            <p className="mt-5 text-sm font-inter text-slate-400 max-w-sm">
              A nonprofit building bridges of hope through education, healthcare, women empowerment and environmental care.
            </p>
          ...
          <div className="lg:col-span-2">
            ...
            <ul className="space-y-2 text-sm font-inter">
              {NAV_LINKS.map((l) => (
                <li key={l}>
                  <button data-testid={`footer-link-${l}`} onClick={() => scrollTo(l)} className="hover:text-green-400 transition-colors capitalize">{l}</button>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div className="lg:col-span-3">
            <p className="font-poppins font-semibold text-white mb-4">Programs</p>
            <ul className="space-y-2 text-sm font-inter">
              {PROGRAM_LINKS.map((p) => (
                <li key={p}><span className="hover:text-green-400 cursor-default">{p}</span></li>
              ))}
            </ul>
          </div>
          ...
          <div className="lg:col-span-3">
            ...
            <p className="text-sm font-inter text-slate-400 mb-4">Monthly stories from the field — no spam.</p>
            <form onSubmit={subscribe} data-testid="newsletter-form" className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-full pl-4 pr-1 py-1">
              <input
                ...
                className="flex-1 bg-transparent text-sm py-2 focus:outline-none placeholder-slate-500"
              />
              <button
                type="submit"
        ...
        <div className="mt-14 pt-8 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-4 text-xs font-inter text-slate-500">
          ...
          <div className="flex gap-6">
            ...
            <a href="#" className="hover:text-green-400">Transparency</a>
          </div>
        </div>
      </div>
    </footer>
